...
const BentoHomeGrid = () => {
    ...
    const mobileLayout = (
      <div className="flex flex-col gap-4 w-full">
         <BentoCard className="md:col-span-2 md:row-span-2 p-4 flex flex-col justify-start animate-expand-y bg-white/10 dark:bg-black/20 text-black dark:text-white" style={{ animationDelay: '0s' }}>
        ...
        <div className="grid grid-cols-2 gap-4">
             <BentoCard className="col-span-1 flex flex-col items-center justify-center animate-expand-x bg-white/10 dark:bg-black/20 text-black dark:text-white" style={{ animationDelay: '0.2s' }}>
                 <h3 className="text-3xl md:text-5xl font-bold">09+</h3>
                 <p className="text-neutral-800 dark:text-neutral-200 text-xs uppercase tracking-wider text-center">Deployed Projects</p>
            </BentoCard>
            <a href={certificatesLink} target="_blank" rel="noopener noreferrer" className="group animate-expand-x col-span-1" style={{ animationDelay: '0.1s' }}>
                <BentoCard className="h-full flex flex-col items-center justify-center cursor-pointer bg-white/10 dark:bg-black/20 text-black dark:text-white">
        ...
        <BentoCard className="col-span-1 md:col-span-2 flex flex-col justify-center items-center animate-expand-y bg-white/10 dark:bg-black/20 text-black dark:text-white" style={{ animationDelay: '0.3s' }}>
        ...
        <div className="grid grid-cols-3 gap-4">
             <a href="https://www.linkedin.com/in/vanshdeep-verma" target="_blank" rel="noopener noreferrer" className="group animate-expand-y" style={{animationDelay: '0.2s'}}>
                        <BentoCard className="h-full items-center justify-center transition-transform group-hover:scale-105 bg-white/10 dark:bg-black/20 text-black dark:text-white">
...
const ProjectsView = ({ onProjectHover }: { onProjectHover: (description: string | null) => void }) => {
    ...
    return (
        <div className={cn("h-full flex flex-col", isMobile && "px-4 pt-4")}>
            <h2 className="text-2xl font-bold text-black dark:text-white mb-4 shrink-0 animate-expand-x">My Works</h2>
            <div className="flex-grow min-h-0 relative">
                <ScrollArea className="absolute inset-0 h-full w-full">
                    <div className={cn("grid auto-rows-fr gap-4 p-1", isMobile ? "grid-cols-1" : "grid-cols-4")}>
                        {projectsData.map(project => (
                            <div
                                key={project.id}
                                className={cn(
                                    project.animation,
                                    !isMobile && `${project.colSpan} ${project.rowSpan}`
                                )}
...
const MobileNav = ({ activeView, setActiveView, navItems }: { activeView: string, setActiveView: (view: string) => void, navItems: any[] }) => (
    <div className="md:hidden fixed top-0 left-0 right-0 z-30 p-2">
        <BentoCard className="flex justify-around items-center py-2 px-2">
            {navItems.map(item => (
                <button
                    key={item.label}
                    onClick={() => setActiveView(item.label)}
                    className={cn(
                        "flex flex-col items-center gap-1 p-1 rounded-lg transition-colors text-xs font-bold",
...
export function GlassPanelLayout() {
  ...
  const renderContent = () => {
    ...
    switch (activeView) {
      ...
      case 'Personal':
        content = (
            <div className={cn("h-full flex flex-col", isMobile && "px-4 pt-4")}>
                <div className={cn("flex flex-col flex-grow min-h-0 animate-expand-y", isMobile ? "bg-transparent text-black dark:text-white" : "bg-white/80 dark:bg-black/70 rounded-[20px] p-4")} style={{ animationDelay: '0s' }}>
  ...
  if (isMobile) {
    return (
      <div className="relative z-20 w-full h-screen flex flex-col items-center justify-start pt-24 p-4">
        <MobileNav navItems={navItems} activeView={activeView} setActiveView={setActiveView} />
