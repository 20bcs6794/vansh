    ...
    if (isMobile) {
        return (
            <div className="flex flex-col gap-4 w-full">
                 <div className="bg-white/10 dark:bg-black/20 rounded-2xl p-6 flex flex-col items-center text-center">
...
const GlassPanel = forwardRef<HTMLDivElement, { className?: string, children: ReactNode, style?: CSSProperties }>(({ className, children, style }, ref) => {
...
const ProjectsView = ({ onProjectHover, isMobile = false }: { onProjectHover: (description: string | null) => void, isMobile?: boolean }) => {
    ...
    const containerClasses = isMobile ? "px-4" : "";

    return (
        <div className={cn("h-full flex flex-col", containerClasses)}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 auto-rows-fr gap-4 p-1">
                        {projectsData.map(project => (
                            <div
                                key={project.id}
                                className={cn(
                                    isMobile ? "col-span-1 row-span-1" : `${project.colSpan} ${project.rowSpan}`,
                                    project.animation
                                )}
            ...
            <div className="flex-grow min-h-0">
                <ScrollArea className="h-full pr-4">
                    <div className="relative flex flex-col gap-y-10">
                         <div className="absolute left-32 top-0 h-full w-1 bg-white/10 dark:bg-black/20 translate-x-1/2 animate-expand-y" />
...
export function GlassPanelLayout() {
  ...
  const renderMobileContent = () => {
    switch (activeView) {
      case 'Home':
        return <BentoHomeGrid isMobile={isMobile} />;
      case 'Projects':
            ...
            <div className="h-full flex flex-col gap-4 px-4 pt-4 text-white">
                ...
                <div className="flex flex-col mt-4">
                    <h3 className="text-xl font-bold text-left shrink-0 mb-2">Tools and Technologies</h3>
                     <div className="group relative w-full overflow-hidden [mask-image:linear-gradient(to_right,transparent_0,black_20px,black_calc(100%-20px),transparent_100%)]">
                        <div className="flex w-max animate-scroll-x gap-4 group-hover:[animation-play-state:paused] p-4">
                            <div className="flex shrink-0 gap-4">
                                {technologiesWithIcons.map((tech, index) => (
                                    <TechnologyCard key={`${tech.name}-${index}`} name={tech.name} icon={tech.icon} />
                                ))}
      ...
      case 'Career':
        return <CareerTimeline isMobile={isMobile} />;
      default:
        return null;
    }
  }
  ...
  if (isMobile) {
    return (
        <div className="relative z-20 w-full h-screen flex flex-col items-center pt-4 text-white">
            ...
            <ScrollArea className="w-full flex-grow pt-4">
                <div className="flex flex-col items-center gap-y-4 pb-4 px-4">
                  {renderMobileContent()}
                </div>
            </ScrollArea>
        </div>
      ...
      <div style={{ perspective: '2000px' }}>
        ...
        >
          <div className="h-auto hidden md:flex items-center">
           ...
           <GlassPanel 
               className="w-[600px] h-[480px] mx-6"
           >
             {renderContent()}
           </GlassPanel>
