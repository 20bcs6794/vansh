...
const BentoHomeGrid = ({ isMobile = false }) => {
    ...
    if (isMobile) {
        return (
            <div className="grid grid-cols-1 gap-4 w-full p-4 text-black dark:text-white">
                <BentoCard className="md:col-span-2 md:row-span-2 p-4 flex flex-col justify-start bg-white/10 dark:bg-black/20">
                ...
                <div className="grid grid-cols-3 gap-4">
                    <a href="https://www.linkedin.com/in/vanshdeep-verma" target="_blank" rel="noopener noreferrer" className="group">
                        <BentoCard className="h-full items-center justify-center transition-transform group-hover:scale-105 bg-white/10 dark:bg-black/20">
...
const ProjectsView = ({ onProjectHover, isMobile = false }: { onProjectHover: (description: string | null) => void, isMobile?: boolean }) => {
    const projectsData = [
      {
        id: 1,
    ...
    const containerClass = isMobile
        ...
        : "grid grid-cols-4 auto-rows-fr gap-4 p-1";

    const colSpan = (p: any) => isMobile ? 'col-span-1' : p.colSpan;
    const rowSpan = (p: any) => isMobile ? 'row-span-1' : p.rowSpan;

    return (
        <div className={cn("h-full flex flex-col", isMobile && "pt-4 px-4")}>
                    ...
                    <div className={containerClass}>
                        {projectsData.map(project => (
                            <div
                                ...
                                className={cn(
                                    project.animation,
                                    `${colSpan(project)} ${rowSpan(project)}`
                                )}
...
const HtmlIcon = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 48 48" {...props}>
        ...
        <path fill="#FF6D00" d="M24 8L24 39.9 35.2 36.7 3.7 8z" />
...
const CareerTimeline = ({ isMobile = false }) => {
    const containerClass = isMobile ? "pt-4 px-4 text-black dark:text-white" : "";
    return (
        <div className={cn("h-full flex flex-col", containerClass)}>
...
const ContactView = () => {
    ...
    return (
        <div className="h-full flex flex-col p-4 gap-4 text-black dark:text-white">
             <h2 className="text-2xl font-bold text-center mb-0 animate-expand-x" style={{animationDelay: '0s'}}>Get In Touch</h2>
            <div className="flex flex-col gap-4 flex-grow">
                <BentoCard className="col-span-2 row-span-1 p-4 animate-expand-y bg-white/10 dark:bg-black/20" style={{animationDelay: '0.1s'}}>
...
const MobileNav = ({ activeView, setActiveView, navItems }: { activeView: string, setActiveView: (view: string) => void, navItems: any[] }) => {
    return (
        <div className="fixed top-0 left-0 right-0 z-30 p-2 bg-background/50 backdrop-blur-sm">
...
export function GlassPanelLayout() {
  ...
  const getPanelStyle = (panel: 'left' | 'right'): CSSProperties => {
    ...
    switch (activeView) {
      case 'Home':
        content = <BentoHomeGrid isMobile={isMobileView} />;
        break;
      case 'Projects':
        content = <ProjectsView onProjectHover={setProjectDescriptionForRightPanel} isMobile={isMobileView} />;
        break;
      case 'Personal':
        content = (
            <div className={cn("h-full flex flex-col", isMobileView && "pt-4 px-4 text-black dark:text-white")}>
                <div className={cn("bg-white/10 dark:bg-black/20 rounded-[20px] p-4 flex flex-col flex-grow min-h-0 animate-expand-y", isMobileView && "bg-transparent dark:bg-transparent") } style={{ animationDelay: '0s' }}>
        ...
        break;
      case 'Career':
        content = <CareerTimeline isMobile={isMobileView} />;
        break;
      default:
        content = null;
    }
  ...
  const originalAboutContent = (
    ...
    if (isMobile) {
        return (
            <div className="w-full h-screen overflow-y-auto pt-16">
